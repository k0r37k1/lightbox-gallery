function init() {
    ...
    function changeImage(direction) {
        ...
        updateTitleAndDescription(currentItem);
        document.getElementById("image-counter").innerText = `${currentItem + 1} / ${items.length}`;
    }

    function resetImageAndAnimation() {
        gsap.to(lightboxImage, {xPercent: 0, yPercent: 0, opacity: 1, rotation: 0, scaleX: 1, scaleY: 1, duration: 0});
        rotation = 0;
        scaleX = 1;
        scaleY = 1;
        scale = 1;
    ...
    function transformImage() {
        gsap.set(document.getElementById("image-wrapper"), {x: translateX, y: translateY, scale: scale});
    }
...
function printImage() {
    ...
    printDocument.write(`
        ...
        <body>
        <img src='${lightboxImage.src}' style='max-width: 100%; max-height: 100%;'>
        </body>
        </html>
    `);
    printDocument.close();
    ...
    const timeoutID = setTimeout(removeIframe, 1000);
}
